...
(async () => {
  ...
  try {
    ...
    let findquery = contractsTokenizados.map((x) => x.contrato_id);

    const sendContracts = await collectionContracts
      .find({ contratoId: { $nin: findquery } })
      .toArray();

    let noFoundNin = sendContracts.map((y) => y.contratoId);

    console.log(noFoundNin);
